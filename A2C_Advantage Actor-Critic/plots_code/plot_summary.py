...
def main():
    ...
    plt.title("__v0.5__ — comparación sticky modes")
    plt.tight_layout()
    plt.savefig(outdir / "__v0.5__sticky_modes_boxplot.png", dpi=160)
    plt.close()
    ...
    plt.ylabel("mean(R_τ) por episodio")
    plt.title("__v0.5__ — efecto de decidir cada T slots")
    plt.legend()
    plt.tight_layout()
    plt.savefig(outdir / "__v0.5__decision_period_effect.png", dpi=160)
    plt.close()
    ...
    plt.title("__v0.5__ POMDP — δ_t vs carga (muestra)")
    plt.tight_layout()
    plt.savefig(outdir / "__v0.5__pomdp_delta_vs_load_scatter.png", dpi=160)
    plt.close()
