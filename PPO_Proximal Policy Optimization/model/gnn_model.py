...
del algoritmo PPO. Emplea capas `HeteroConv` basadas en convoluciones atencionales (GATv2Conv) 
...
class GNN(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict=None, batch_dict=None):
        ...
        # 4. Cabeza de Valor (Critic, para PPO)
        ...
        if batch_dict is not None and 'ap' in batch_dict:
            ...
            # Asumimos que es un solo grafo si no hay batch_dict
            graph_emb = ap_emb.mean(dim=0, keepdim=True)
            
        state_value = self.value_head(graph_emb)
