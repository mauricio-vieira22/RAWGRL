...
def run_test():
    ...
    import joblib
    dists = joblib.load("data/distributions_990.joblib")
    print(f"\nCargados {len(dists)} clientes.\n")

    # 2. Generar simulación muy corta
    adm = ArrivalDepartureModel(dists, arrival_rate=1.0, mean_duration=10.0, total_timesteps=1, random_seed=42)
    ...
    grilla_obj = crear_grilla(eventos, dists, total_timesteps=1)
    ...
    # 5. Volcar Resultados en Tabla
    print(f"\n--- TIMESTEP 0 --- (Sigma [Ruido Térmico] = {SIGMA_DBM} dBm, es decir {db_to_linear(torch.tensor(SIGMA_DBM)).item():.2e} mW)")
    ...
    for cli_idx in range(len(eventos)):
        ap_idx    = asignaciones[cli_idx, 0, 0].item()
        band_idx  = asignaciones[cli_idx, 0, 1].item()
        rssi_val  = asignaciones[cli_idx, 0, 2].item()
        sinr_val  = sinr_lin[cli_idx, 0].item()
        rate_val  = rate[cli_idx, 0].item()
