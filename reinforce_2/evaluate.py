r"""
evaluate.py — Inferencia Empírica y Evaluación de Políticas (NetROML).

Este módulo ejecuta la política Actor $\pi_{\theta^*}$ pre-entrenada para evaluar su rendimiento 
sobre instanciaciones estocásticas independientes del POMDP de la red Wi-Fi.
...
def run_episode(
    ...
    with torch.no_grad():
        while not done:
            ...
            ch_acts = acts % env.n_channels
            pwr_acts = acts // env.n_channels
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--decision_period", type=int,   default=25,
