...
class ArrivalDepartureModel:
    ...
    def get_delta_t(self, timestep: int) -> int:
        ...
        Si no hay más arribos, δ_t = T - t (horizonte restante), lo que
        permite distinguir este caso de δ_t = 1 (arribo en el slot siguiente).

        Returns
        -------
        int — Slots hasta el próximo arribo. T - t si no hay más arribos.
        """
    
        if self._delta_t_array is not None:
            if timestep >= self.total_timesteps:
                return 0  # fuera del horizonte: no hay más nada
            return int(self._delta_t_array[timestep])  # sin clamp
        nexts = [e.arrival_time for e in self.events if e.arrival_time > timestep]

        return (min(nexts) - timestep) if nexts else (self.total_timesteps - timestep)
    ...
    def _build_delta_t_array(self):
        ...
        if not arrivals:
            # Sin arribos: δ_t = T - t para todo t (horizonte restante)
            delta = np.array([T - t for t in range(T)], dtype=np.int32)
        ...
        for t in range(T):
            while j < len(arrivals) and arrivals[j] <= t:
                j += 1
            # próximo arribo existe: distancia real
            # no existe: T - t como centinela distinguible
            delta[t] = (arrivals[j] - t) if j < len(arrivals) else (T - t)
