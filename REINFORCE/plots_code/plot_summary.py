...
def main():
    ...
    plt.title("__v0__ — comparación sticky modes")
    plt.tight_layout()
    plt.savefig(outdir / "__v0__sticky_modes_boxplot.png", dpi=160)
    plt.close()
    ...
    plt.ylabel("mean(R_τ) por episodio")
    plt.title("__v0__ — efecto de decidir cada T slots")
    plt.legend()
    plt.tight_layout()
    plt.savefig(outdir / "__v0__decision_period_effect.png", dpi=160)
    plt.close()
    ...
    plt.title("__v0__ POMDP — δ_t vs carga (muestra)")
    plt.tight_layout()
    plt.savefig(outdir / "__v0__pomdp_delta_vs_load_scatter.png", dpi=160)
    plt.close()
