...
class RandomModel(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict=None, batch_dict=None):
        ...
        logits = torch.zeros((n_aps, self.out_channels_ch * self.out_channels_pwr), device=device) + self.dummy
        return logits
...
def train(args: argparse.Namespace) -> tuple[torch.nn.Module, pd.DataFrame]:
    """
    ...
    for episode in range(args.episodes):
        ...
        while not done:
            ...
            log_probs_list.append(dist.log_prob(acts).mean())
            entropies_list.append(dist.entropy().mean())
